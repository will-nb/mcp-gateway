...
def fetch_by_isbn(
    ...
    *,
    ...
    if source == SOURCE_GOOGLE_BOOKS:
        return google_books.fetch_by_isbn(
            isbn, api_key=api_key, lang=lang, timeout=timeout
        )
...
def search_by_title(
    ...
    *,
    ...
    if source == SOURCE_GOOGLE_BOOKS:
        return google_books.search_by_title(
            title, api_key=api_key, lang=lang, max_results=max_results, timeout=timeout
        )
    if source == SOURCE_OPEN_LIBRARY:
        return open_library.search_by_title(
            title, max_results=max_results, timeout=timeout
        )
    if source == SOURCE_LOC:
