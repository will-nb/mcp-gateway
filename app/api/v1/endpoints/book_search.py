...
def search_books_by_title(
    ...
    s = get_settings()
    task_req = EnqueueRequest(
        payload_ref="inline://books_search_title", params=req.model_dump()
    )
    ...
    if (
        doc
        and doc.get("status") == JobStatus.succeeded
        and doc.get("result") is not None
    ):
        return create_object_response(
            message="OK",
            data_value={"items": doc.get("result")},
...
def search_books_by_isbn(
    ...
    s = get_settings()
    task_req = EnqueueRequest(
        payload_ref="inline://books_search_isbn", params=req.model_dump()
    )
    job_id, _ = enqueue_task(
