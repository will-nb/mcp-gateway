...
def test_qdrant_endpoints(monkeypatch):
    ...
    class FakeQdrantClient:
        ...
        def upsert(self, collection_name, points, wait=True):
            return types.SimpleNamespace(
                status=types.SimpleNamespace(value="acknowledged")
            )
        ...
        def query_points(
            ...
            class Rec:
                def __init__(self):
                    self.points = [
                        types.SimpleNamespace(id=1, score=0.99, payload={"k": "v"})
                    ]
    ...
    # ensure collection
    r = client.post(
        "/api/v1/qdrant/ensure-collection",
        json={"collection_name": "foo", "vector_size": 4},
    )
    ...
    # upsert
    r = client.post(
        "/api/v1/qdrant/upsert",
        json={
            "collection_name": "foo",
            "points": [
                {"id": 1, "vector": [0.1, 0.2, 0.3, 0.4], "payload": {"k": "v"}}
            ],
        },
    )
    assert r.status_code == 200
    ...
    # search
    r = client.post(
        "/api/v1/qdrant/search",
        json={"collection_name": "foo", "vector": [0.1, 0.2, 0.3, 0.4], "limit": 1},
    )
    assert r.status_code == 200
    data = r.json()["data"]
