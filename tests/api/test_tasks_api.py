...
def test_tasks_enqueue_ocr_accepts_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.tasks.enqueue_task", lambda **kw: ("job-123", "queued")
    )
...
def test_tasks_enqueue_rejects_callback_outside_whitelist(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.endpoints.tasks.enqueue_task", lambda **kw: raise_error(**kw)
    )
...
def test_jobs_status_and_cancel(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda job_id: {"id": job_id, "status": "queued"},
    )
